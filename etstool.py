...
def test(runtime, toolkit, environment):
    ...
    commands = [
        "edm run -e {environment} -- coverage run -p -m nose.core -v ensemble --nologcapture"]
    # We run in a tempdir to avoid accidentally picking up wrong ensemble
...
def get_parameters(runtime, toolkit, environment):
    ...
    if environment is None:
        parameters['environment'] = 'ensemble-test-{runtime}-{toolkit}'.format(**parameters)
    return parameters
